# usage:
# R --slave --vanilla --file=DEASMAACCRPreferenceRelationsCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
pairwiseOutrankingIndicesFile <- "pairwiseOutrankingIndices.xml"
...
source("ImprecisePairwiseComparisonsSMAA.R")
results <- handleException(
  function() return(
    calculateComparisonsSMAAForAll(inputs, inputs$samplesNo)
  ),
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate DEASMAACCRPreferenceRelations.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert DEASMAACCRPreferenceRelations results into XMCDA")
}

...
inputsOutputsFunctionsFile <- "inputsOutputsFunctions.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, inputsOutputsFunctionsFile, mandatory = FALSE, xmcdaMessages, "criteriaFunctions")
...
source("ImprecisePairwiseComparisonsSMAA-VDEA.R")
results <- handleException(
  function()
    return(
      calculateComparisonsSMAAForAll(inputs, inputs$samplesNo, inputs$transformToUtilities)
    )
  ,
  xmcdaMessages,

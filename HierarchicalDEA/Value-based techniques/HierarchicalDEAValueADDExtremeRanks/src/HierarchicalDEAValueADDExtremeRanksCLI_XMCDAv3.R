...
setwd(script.dir())
source("rank-hierarchy.R")
results <- handleException(
  function()
  {
    return(calculateRank(dmuData, dmuData$transformToUtilities))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEAValueADDExtremeRanks")
}

# fine, now let's put the results into XMCDA structures

xResults = convert(results, dmuData$altIDs, xmcdaMessages)

if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEAValueADDExtremeRanks results into XMCDA")
}

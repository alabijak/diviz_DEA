...
results <- handleException(
  function()
  {
    return(calculateEfficiencyBoundsForAll(dmuData, dmuData$transformToUtilities))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEAValueADDEfficiency")
}
...
if (is.null(xResults)){
  writeXMCDAv2(xmcdaMessages, paste(outDirectory, messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEAValueADDEfficiency results into XMCDA")
}

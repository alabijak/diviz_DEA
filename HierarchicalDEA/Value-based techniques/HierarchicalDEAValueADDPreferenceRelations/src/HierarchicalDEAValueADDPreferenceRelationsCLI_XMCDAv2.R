...
setwd(script.dir())
source("efficiencyDominance-hierarchy.R")
results <- handleException(
  function()
  {
    return(calculateEfficiencyDominanceForAll(dmuData, dmuData$transformToUtilities))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDAv2(xmcdaMessages, paste(outDirectory, messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEAValueADDPreferenceRelations")
}
...
if (is.null(xResults)){
  writeXMCDAv2(xmcdaMessages, paste(outDirectory, messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEAValueADDPreferenceRelations results into XMCDA")
}

#! /bin/bash
# Usage:
#  HierarchicalDEASMAAValueADDEfficiency.sh [--v2|--v3] input_dir output_dir [random_seed]
...
if [ $# -lt 3 ]; then
  ...
  R --slave --vanilla --file=src/HierarchicalDEASMAAValueADDEfficiencyCLI_XMCDAv3.R --args $2 $3 $4
  ret=$?
elif [ $1 = "--v2" ]; then
  R --slave --vanilla --file=src/HierarchicalDEASMAAValueADDEfficiencyCLI_XMCDAv2.R --args $2 $3 $4
  ret=$?
else
  echo "Usage: $0 [--v2|--v3] input_dir output_dir [random_seed]" >&2
  exit 4

...
setwd(script.dir())
source("pairwiseComparisonsSMAA-VDEA-hierarchy.R")
results <- handleException(
  function()
  {
    return(calculateComparisonsSMAAForAll(dmuData, dmuData$samplesNo, dmuData$transformToUtilities))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEASMAAValueADDPreferenceRelations")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEASMAAValueADDPreferenceRelations results into XMCDA")
}

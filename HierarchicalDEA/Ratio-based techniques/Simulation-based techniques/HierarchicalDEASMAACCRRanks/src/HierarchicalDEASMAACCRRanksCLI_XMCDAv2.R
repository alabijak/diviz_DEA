...
setwd(script.dir())
source("rankSMAAHierarchical.R")
results <- handleException(
  function()
  {
    return(calculateRankSMAAForAll(dmuData, dmuData$samplesNo))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEASMAACCRRanks")
}
...
if (is.null(xResults)){
  writeXMCDAv2(xmcdaMessages, paste(outDirectory, messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEASMAACCRRanks results into XMCDA")
}

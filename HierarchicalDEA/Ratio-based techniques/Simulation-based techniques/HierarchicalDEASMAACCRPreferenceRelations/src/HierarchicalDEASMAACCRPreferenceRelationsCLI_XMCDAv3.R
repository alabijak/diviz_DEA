...
setwd(script.dir())
source("pairwiseComparisonsSMAAHierarchical.R")
results <- handleException(
  function()
  {
    return(calculateComparisonsSMAAForAll(dmuData, dmuData$samplesNo))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEASMAACCRPreferenceRelations")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEASMAACCRPreferenceRelations results into XMCDA")
}

...
setwd(script.dir())
source("HierarchicalDEARanks.R")
results <- handleException(
  function()
  {
    return(calculateRanksForAll(dmuData))
  }
  ,
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate HierarchicalDEACCREfficiency")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert HierarchicalDEACCREfficiency results into XMCDA")
}
